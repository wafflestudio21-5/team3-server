...
class JwtAuthenticationFilter(private val jwtTokenProvider: JwtTokenProvider) : Filter {
    ...
    private fun isExcludedPath(path: String): Boolean {
        return path.startsWith("/oauth2") || path.startsWith("/login") || path.startsWith("/api/signin") || path.startsWith("/api/signup") || path.startsWith("/test-page") || path.contains("/swagger-ui/") || path.contains("/v3/api-docs") || path.startsWith("/login/oauth2")
    }
}